...
namespace Plais.Services
{
        ...
        IAchievementRepository achievementRepository) : IAchievementService
    {
        public async Task<Achievement> CreateAsync(SaveAchievementDto dto)
        {
            ...
            await achievementRepository.IncrementAllPositionsAsync();
        ...
        public async Task MoveDownAsync(int id)
        {
            ...
            var targetPos = achievement.Position + 1;

            var toSwap = await achievementRepository.GetByPositionAsync(targetPos);
            if (toSwap == null) return;

            toSwap.Position = achievement.Position;
            achievement.Position = targetPos;
        ...
        public async Task MoveUpAsync(int id)
        {
            ...
            if (achievement.Position <= 0) return;

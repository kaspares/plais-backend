...
namespace Plais.Services
{
        ...
        IEventGroupRepository eventGroupRepository) : IEventGroupService
    {
        public async Task<EventGroupDto> CreateAsync(SaveEventGroupDto dto)
        {
            ...
            eventGroup.Position = 0;
            eventGroup.DateCreated = DateTime.Now;

            await eventGroupRepository.IncrementAllPositionsAsync();
        ...
        {
            var group = await eventGroupRepository.GetByIdAsync(id)
            ?? throw new NotFoundException(nameof(EventGroup), id.ToString());

            var targetPos = group.Position + 1;

            var toSwap = await eventGroupRepository.GetByPositionAsync(targetPos);
            if (toSwap == null) return;

            toSwap.Position = group.Position;
        ...
        public async Task UpdateAsync(int id, EditEventGroupDto dto)
        {

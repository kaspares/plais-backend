...
namespace Plais.Services
{
        ...
        IEventRepository eventRepository) : IEventService
    {
        public async Task<EventDto> CreateAsync(SaveEventDto dto)
        {
            ...
            eventEntity.Position = 0;
            eventEntity.DateCreated = DateTime.Now;

            await eventRepository.IncrementAllEventPositionsInGroupAsync(dto.EventGroupId);
        ...
        public async Task MoveDownAsync(int id)
        {
            ...
            var targetPos = eventEntity.Position + 1;

            var toSwap = await eventRepository.GetByPositionAsync(targetPos);
            if (toSwap == null) return;

            toSwap.Position = eventEntity.Position;
            eventEntity.Position = targetPos;
        ...
        public async Task MoveUpAsync(int id)
        {
            ...
            if (eventEntity.Position <= 0) return;
        ...
        public async Task UpdateAsync(int id, EditEventDto dto)
        {

...
namespace Plais.Data.Repositories
{
    public class EventGroupRepository(PlaisDbContext dbContext) : IEventGroupRepository
    {
        ...
        public async Task<EventGroup?> GetByPositionAsync(int position)
        {
            var eventGroup = await dbContext.EventGroups.FirstOrDefaultAsync(e => e.Position == position);
            return eventGroup;
        }

        public async Task IncrementAllPositionsAsync()
        {
            await dbContext.EventGroups.ExecuteUpdateAsync(s => s.SetProperty(e => e.Position, e => e.Position + 1));
